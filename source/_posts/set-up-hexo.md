...
启动成功后，可以打开浏览器，在地址栏输入 `http://localhost:4000`，就可以看到我们搭建的博客啦。
...
需要注意的是，在 `/tags/ || tags` 中，`/tags` 表示网站目录，`tags` 表示使用的图标，可以在 [Font Awesome Icons](https://fontawesome.com/v4.7.0/icons/) 选择喜欢的图标，将图标名填写在 `||` 后即可。
